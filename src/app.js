...
export class App{
  ...
  async exportBest(){const best=State.data.best;if(!best.genome){this.log('Kein Ergebnis');return;}const payload={ts:new Date().toISOString(),fitness:Number(best.fitness.toFixed(6)),genome:best.genome,target:State.data.target,version:State.data.version};const line=JSON.stringify(payload);const h=await sha256(line);download(`best-${payload.ts.replace(/[:.]/g,'-')}-${h.slice(0,8)}.jsonl`,line+'\n');this.log('Exportiert');}
