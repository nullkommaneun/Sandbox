...
export async function sha256(text){
  try{ if(crypto && crypto.subtle){ const b=new TextEncoder().encode(text); const d=await crypto.subtle.digest('SHA-256', b); return [...new Uint8Array(d)].map(x=>x.toString(16).padStart(2,'0')).join(''); } }catch{}
...
export function fmtArr(arr){ return '[' + arr.map(v=>Number(v).toFixed(3)).join(', ') + ']'; }
